...
> **OpenWeatherMap 실시간 기상 데이터와 6대 국가산업단지 설비 물리 손상 모델을 결합한 B2B 스마트 공정 안전 관리 플랫폼**

---

## 1. 프로젝트 개요

* **개발 목적**: 폭염, 집중호우, 혹한, 낙뢰, 미세먼지 등 기상이변 발생 시 대한민국 6대 국가산업단지(창원, 울산, 군산, 광주, 포항, 여수)의 핵심 설비(CNC 공작기계, 옥외 배관, 대형 프레스 유압유, 반도체 클린룸) 파손을 사전에 방지하고 표준 작업 절차(SOP)를 가동하기 위한 관제 시스템입니다.
* **기술 스택**: Vue 3 (`<script setup>`), Vue Router 4, Pinia, Axios, UI 라이브러리(Element Plus, Ant Design Vue), SVG 반응형 시각화.

---

## 2. Vue.js 교육과정(1~10단원) 기능 매핑 명세표

교재 10대 단원 전 범위의 문법과 실습 예제를 100% 반영하여 구현했습니다.
...
* **테스트 내용**:
  1. **인터랙티브 미니 시뮬레이터**: 외기 기온(15~42℃) 및 습도(20~98%) 슬라이더를 움직이면 창원 CNC 열변형(+μm), 울산 배관 부식도(%), 군산 프레스 작동유 온도(℃), 광주 클린룸 상태가 실시간으로 재계산되어 표시됩니다.
...
* **테스트 내용**:
  1. **기술 아카이브**: UI 갤러리, 5대 난제 트러블슈팅, 물리 공식, 비전공자 가이드를 탭별로 확인합니다.
